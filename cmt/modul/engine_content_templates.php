...
if (isset($_POST['cmt_save'])) {
	...
	if ($_POST['cmt_save'] != 'reload'){
		...
		$_POST['next_row'] = (isset($_POST['next_row'])) ? 1 : 0;
		
		if (!isset($_POST['title']) OR !$_POST['title']) $alert['title'] = 'error';
		if (isset($_POST['label'])) $_POST['label'] = strtoupper(str_replace(" ", "_", $_POST['label']));
		if (isset($_POST['label_before'])) $_POST['label_before'] = strtoupper(str_replace(" ", "_", $_POST['label_before']));
		if (isset($_POST['label_after'])) $_POST['label_after'] = strtoupper(str_replace(" ", "_", $_POST['label_after']));
		if (isset($_POST['label_head'])) $_POST['label_head'] = strtoupper(str_replace(" ", "_", $_POST['label_head']));
		
		if (isset($_POST['max_columns'])){
			if (!is_numeric($_POST['max_columns'])) $alert['max_columns'] = 'error';
			if (strlen($_POST['max_columns']) < 1) $alert['max_columns'] = 'error';
			if (strlen($_POST['max_columns']) > 7) $alert['max_columns'] = 'error';
		}
		
		if (isset($_POST['max_width'])){
			if (!is_numeric($_POST['max_width'])) $alert['max_width'] = 'error';
		}
		
		if (isset($_POST['max_rows'])){
			if (!is_numeric($_POST['max_rows'])) $alert['max_rows'] = 'error';
			if (strlen($_POST['max_rows']) < 1) $alert['max_rows'] = 'error';
		}
...
if (isset($id_field)) cross_table('id_field', $id_field);
else cross_table('id_field');
$formdata['id_field']['settings']['data'][0] = v('CMT_BLOCK');

if (isset($id_field)){
	if (!isset($label)) $label = $title;
	if (!isset($next_row)) $next_row = 1;
	if (!isset($max_columns)) $max_columns = 0;
	if (!isset($max_width)) $max_width = 0;
	...
	if ($id_field != 0){
		...
		$formdata['label_before'] = array	(
			'type'				=>	'text',
			'label'				=>	v('CMT_LABEL'),
			'label_after'	=>	v('CMT_LABEL_BEFORE'),
			'style'				=>	array(
				'class'			=>	"col-3_of_7"
			)
		);
		
		$formdata['label_after'] = array	(
			'type'				=>	'text',
			'label'				=>	v('CMT_LABEL'),
			'label_after'	=>	v('CMT_LABEL_AFTER'),
			'style'				=>	array(
				'class'			=>	"col-3_of_7"
			)
		);
		
		$formdata['label_head'] = array	(
			'type'				=>	'text',
			'label'				=>	v('CMT_LABEL'),
			'label_after'	=>	v('CMT_LABEL_HEAD'),
			'style'				=>	array(
				'class'			=>	"col-3_of_7"
			)
		);
		...
		if ($field_type != 'datepicker'){
			...
			if ($field_type == 'int'){
				if (isset($align) && $align == 'none') $align = 'right';
			} else if (isset($align) && $align == 'none') $align = 'left';
			
			if ($field_type != 'select'){
				$formdata['align'] = array	(
					'type'				=>	'select',
					'label'				=>	v('CMT_ALIGN'),
					'settings'		=>	array(
						'data'			=>	array(
							'left'		=>	v('CMT_ALIGN_LEFT'),
							'right'		=>	v('CMT_ALIGN_RIGHT'),
							'center'	=>	v('CMT_ALIGN_CENTER')
						)
					),
					'style'				=>	array(
						'class'			=>	"col-3_of_7"
			...
			$formdata['next_row'] = array	(
				'type'					=>	'checkbox',
				'label'					=>	v('CMT_NEXT_ROW')
			);
			
			$formdata['max_columns'] = array	(
				'type'					=>	'text',
				'label'					=>	v('CMT_COLUMNS'),
				'label_after'		=>	'1-7',
				'style'					=>	array(
			...
			$formdata['max_width'] = array	(
				'type'					=>	'text',
				'label'					=>	v('CMT_WIDTH'),
				'label_after'		=>	v('CMT_PIXEL'),
				'style'					=>	array(
					'class'				=>	'align-right col-1_of_7'
				)
			);
